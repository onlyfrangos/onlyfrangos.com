...
async function compressImage(file: File) {
  ...
  const blob = await new Promise<Blob>((resolve, reject) =>
    canvas.toBlob((value) => value ? resolve(value) : reject(new Error("Não foi possível comprimir a imagem")), "image/webp", 0.82)
  );
  if (blob.size > MAX_FILE_SIZE) throw new Error(`A imagem ${file.name} permaneceu acima de 5 MB após a compressão`);
