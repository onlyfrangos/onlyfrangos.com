...
import { Bell, Bookmark, Compass, Dumbbell, Flame, Home, LogOut, Medal, Menu, MessageCircle, Plus, User2, Users, X } from "lucide-react";
...
export function ProfileMobileNavigation({ username }: ProfileSidebarProps) {
  ...
  return (
    <>
      {isOpen ? (
        <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm lg:hidden" role="presentation" onClick={() => setIsOpen(false)}>
          <aside
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setIsOpen(false)} aria-label="Fechar menu" className="rounded-lg p-2 text-of-muted hover:bg-white/5 hover:text-of-text">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <nav className="grid grid-cols-2 gap-2">
              {sidebarItems.map((item) => {
                const Icon = item.icon;
                return (
                  <Link key={item.label} href={item.href} onClick={() => setIsOpen(false)} className="flex items-center gap-2 rounded-xl border border-of-border/70 px-3 py-3 text-sm text-of-muted hover:bg-white/5 hover:text-of-text">
                    <Icon className="h-4 w-4" />
                    {item.label}
                    {item.badge ? <span className="ml-auto rounded-full bg-red-600 px-1.5 text-[10px] text-white">{item.badge}</span> : null}
                  </Link>
                );
              })}
            </nav>

            <button type="button" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-of-primary px-4 py-3 text-sm font-semibold text-white hover:bg-of-primaryHover">
              <Plus className="h-4 w-4" />
              Criar publicacao
            </button>

            <div className="mt-3 flex items-center gap-2 border-t border-of-border pt-3">
              <Link href={`/${username}`} onClick={() => setIsOpen(false)} className="flex min-w-0 flex-1 items-center gap-2 rounded-xl px-2 py-2 text-sm text-of-text hover:bg-white/5">
                <User2 className="h-4 w-4 shrink-0" />
                <span className="truncate">@{username}</span>
              </Link>
              <button type="button" onClick={() => void handleLogout()} disabled={isLoggingOut} className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-red-400 hover:bg-white/5 disabled:cursor-wait disabled:opacity-60">
                <LogOut className="h-4 w-4" />
                {isLoggingOut ? "Saindo..." : "Sair"}
              </button>
            </div>
      ...
      <nav className="fixed inset-x-3 bottom-3 z-30 rounded-2xl border border-of-border bg-of-surface/95 p-2 backdrop-blur lg:hidden" aria-label="Navegacao mobile">
        <ul className="grid grid-cols-4 gap-1 text-center text-xs text-of-muted">
          <li><Link href="/feed" className="flex flex-col items-center gap-1 rounded-lg px-2 py-1.5 hover:bg-black/25 hover:text-of-text"><Home className="h-4 w-4" />Inicio</Link></li>
          <li><Link href="#" className="flex flex-col items-center gap-1 rounded-lg px-2 py-1.5 hover:bg-black/25 hover:text-of-text"><Compass className="h-4 w-4" />Explorar</Link></li>
          <li><Link href={`/${username}`} className="flex flex-col items-center gap-1 rounded-lg px-2 py-1.5 hover:bg-black/25 hover:text-of-text"><User2 className="h-4 w-4" />Perfil</Link></li>
          <li><button type="button" onClick={() => setIsOpen(true)} aria-expanded={isOpen} className="flex w-full flex-col items-center gap-1 rounded-lg px-2 py-1.5 hover:bg-black/25 hover:text-of-text"><Menu className="h-4 w-4" />Menu</button></li>
        </ul>
      </nav>
    </>
  );
