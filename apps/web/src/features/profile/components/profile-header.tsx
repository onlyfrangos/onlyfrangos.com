...
import { Check, MapPin, Sprout, Edit2, Share2 } from "lucide-react";
  ...
  return (
    <section className="rounded-2xl border border-of-border bg-of-surface/90 px-4 py-4 sm:px-6 sm:py-5">
      <div className="flex flex-col gap-4 border-b border-of-border pb-5 md:flex-row md:items-start md:justify-between">
        <div className="flex items-center gap-4">
          <div className="relative h-24 w-24 overflow-hidden rounded-full border-2 border-red-500 bg-of-surface sm:h-28 sm:w-28">
            <Image
              ...
              priority
            />
            <span className="absolute bottom-1 right-1 h-4 w-4 rounded-full border-2 border-of-surface bg-emerald-500" aria-hidden="true" />
          </div>

          <div>
            <h1 className="flex items-center gap-1 text-1xl font-semibold text-of-text sm:text-2xl">
              {name}
              {verified ? <Check className="h-4 w-4 rounded-full bg-red-600 p-0.5 text-white" aria-label="Perfil verificado" /> : null}
            </h1>
        ...
        <ProfileActions actionMode={actionMode} />
      </div>
      ...
      {bio ? <p className="mt-4 text-base text-of-text sm:text-lg">{bio}</p> : null}
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-2">
          {gymLabel ? <p className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-red-400" />{gymLabel}</p> : null}
          {naturalLabel ? <p className="inline-flex items-center gap-1.5"><Sprout className="h-3.5 w-3.5 text-green-500" />{naturalLabel}</p> : null}
        </div>
...
function ProfileActions({ actionMode }: ProfileActionsProps) {
  if (actionMode === "visitor") {
    return (
      <div className="mt-2 flex items-center gap-2 md:mt-0">
        ...
        <button type="button" className="rounded-lg border border-of-border px-3 py-2 text-sm text-of-text hover:bg-white/10">
          Mensagem
        </button>
        <button type="button" aria-label="Mais opcoes" className="rounded-lg border border-of-border px-2 py-2 text-of-muted hover:text-of-text">
          ...
        </button>
      </div>
    );
