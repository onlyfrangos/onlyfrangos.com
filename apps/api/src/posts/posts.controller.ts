...
export class PostsController {
  ...
  private validateFiles(files: ImageFile[], required: boolean) {
    if ((required && files.length === 0) || files.length > 4)
      throw new BadRequestException('Envie de 1 a 4 imagens');
    if (
      files.some((file) => !allowedTypes.includes(file.mimetype) || !this.hasValidSignature(file))
    )
